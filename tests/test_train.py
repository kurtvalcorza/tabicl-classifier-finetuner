...
def test_resolve_base_model_custom_provided_provenance(tmp_path, monkeypatch):
    ...
    assert matches is False  # random bytes != pinned default
...
def test_resolve_task_type_chain(monkeypatch):
    ...
    assert train._resolve_task_type({}) == "tabular_classification"
...
def test_data_relative_is_relative_to_data(monkeypatch):
    ...
    p = Path("/data/fine-tuning/run123/tabicl_classifier/checkpoints/best.ckpt")
    assert train._data_relative(p) == "fine-tuning/run123/tabicl_classifier/checkpoints/best.ckpt"
