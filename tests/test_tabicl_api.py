...
def test_finetuned_classifier_api_surface():
    from tabicl import FinetunedTabICLClassifier

    params = set(inspect.signature(FinetunedTabICLClassifier.__init__).parameters)
    required = {
        ...
        "verbose", "support_many_classes",
    }
    assert required <= params, f"missing from FinetunedTabICLClassifier: {required - params}"


def test_plain_classifier_api_surface():
    from tabicl import TabICLClassifier

    params = set(inspect.signature(TabICLClassifier.__init__).parameters)
    required = {"model_path", "allow_auto_download", "n_estimators", "random_state", "device", "support_many_classes"}
    assert required <= params, f"missing from TabICLClassifier: {required - params}"


def test_finetuned_classifier_instantiates_with_train_kwargs():
    from tabicl import FinetunedTabICLClassifier
