...
DIMER fine-tuner for the TabICLv2 classifier pipeline. It fine-tunes
[`jingang/TabICL`](https://huggingface.co/jingang/TabICL) (classifier checkpoint) through
`tabicl.FinetunedTabICLClassifier` on a validated tabular-classification dataset, then writes the
fine-tuner artifacts and a `result.json` with metrics and provenance (see [Outputs](#outputs)).
See the [model card](https://github.com/kurtvalcorza/tabicl-classifier-pipeline/blob/main/MODEL_CARD.md)
...
- Pairs with `tabicl-classifier-dataset-validator`.

## Fine-tuning is GPU-only

This component is a **fine-tuner**, not a zero-shot server. `FinetunedTabICLClassifier` requires
...
checkpoint and the training context — see [Outputs](#outputs).
...
- base checkpoint `tabicl-classifier-v2-20260212.ckpt`, pinned by HF revision + SHA-256 and
  verified at fit time; a DIMER-provided base overrides it.
- ordinal-encodes categorical feature columns (upstream fine-tune fails on raw strings).
- creates a **stratified** validation holdout when `val.csv` is absent; stratified-caps training to
  `max_train_rows`.
- scores `test.csv` when present, after best-checkpoint selection (never used for selection).
...
`DIMER_OUTPUT_DIR/tabicl_classifier/`:
...
- `model_id` is intentionally **not** a manifest parameter; the DIMER Base Model selection is
  authoritative.
- The image bakes `DIMER_TASK_TYPE=tabular_classification` as the fallback for DIMER Custom / Other
  pipelines.
